...
def main():
    ...
    for j in ["1А", "1Б", "2А", "2Б", "3А", "3Б", "4А", "4Б", "5А", "5Б", "5В", "6А", "6Б", "6В", # Перебор классов
              "7А", "7Б", "7В", "8А", "8Б", "8В", "9А", "9Б", "9В", "10А", "10Б", "10В", "11А", "11Б", "11В"]:
        for i in open(f"D:/code/python/Flask_project/db/Grades/{j}.txt", encoding="utf-8").readlines(): # Перебор ФИ в файле
    ...
    # Список классов, пользователей которых нужно удалить
    # grades_to_delete = ["1А", "2А", "2Б", "3А", "3Б", "3В", "4А", "4Б", "4В", "5А", "5Б", "5В", "6А", "6Б", "6В",
    #                    "7А", "7Б", "7В", "8А", "8Б", "8В", "9А", "9Б", "9В", "10А", "10Б", "10В", "11А", "11Б", "11В"]

    # Проход по каждому классу в списке
    # for grade in grades_to_delete:
        # Найти всех учеников, у которых класс совпадает с текущим классом из списка
        # pupils_to_delete = db_sess.query(Pupils).filter(Pupils.grade == grade).all()
        
        # Удалить каждого найденного ученика
    ...
    # Удаление записей о добавлении
    # db_sess = db_session.create_session()  # Создание сессии для доступа к БД
    # to_delete = db_sess.query(Kilograms).all()
    # for i in to_delete:
    #     db_sess.delete(i)
    # db_sess.commit()
    # db_sess.close()
    ...
    # Удаление пользователей
    # db_sess = db_session.create_session()  # Создание сессии для доступа к БД
    # me = db_sess.query(User).filter(User.name == "Рудаков Даниил").one()
    # me.got = 0
    # me.got_pupils = 0
    # db_sess.commit()
    # db_sess.close()
